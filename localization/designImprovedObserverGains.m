...
% feasibility even when the solver returns a numerical-difficulty status.
    ...
    fixturePath = string(tempname)+".json";
    fid=fopen(fixturePath,'w'); assert(fid>=0);
    fprintf(fid,'%s\n',jsonencode(fixture)); fclose(fid);
    cleanup=onCleanup(@() delete(fixturePath));
    timer = designLidarOnlyTimerCertificate(fixturePath,InvariantScale=1, ...
        MinimumInterval=cfg.measurement.minimumPoseInterval, ...
        MaximumInterval=cfg.measurement.maximumPoseInterval);
    ...
    design = struct('kind',"aperiodic-pose-v1",'K',data.K,'N',fixture.N, ...
