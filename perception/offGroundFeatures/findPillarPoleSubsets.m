function evidence=findPillarPoleSubsets(points,pillarIds,geometry,cfg,evaluate)
% findPillarPoleSubsets: Find a compact continuous shaft inside a mixed pillar.
...
% semantic point mask is created. Output Gaussian moments remain all-point.
    ...
    if isfield(cfg,'useNativeKernels') && cfg.useNativeKernels
        fields={'maximumSeeds','seedSeparation','fitRadius','fitHeight','heightHypotheses', ...
            'neighborhoodRadius','minimumPoints','minimumOwnPoints','minimumOwnHeight', ...
            'minimumHeight','minimumRobustHeight','maximumTiltDegrees','maximumGap', ...
            'rangeGapScale','heightHalfWindow','minimumWindowPoints','minimumContrast', ...
            'minimumPeakContrast','maximumRadialRms','saturatedHeight','saturatedPoints','saturatedContrast'};
        parameters=cellfun(@(name)double(cfg.(name)),fields);
        measured=perceptionKernelsMex('poleSubsets',double(points),double(group),ids, ...
            double(geometry.mapSize),double(geometry.cellSize),parameters,double(cfg.radii),logical(evaluate(:)));
        for k=1:numel(names), evidence.(names{k})=measured(:,k); end
