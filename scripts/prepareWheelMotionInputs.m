...
% clock is the shared affine model returned by loadReceiverClock.
...
% Missing or expired wheel data cause an error, never a speed-source fallback.
    ...
    assert(endTime>startTime,'VehicleLocalization:InvalidMotionClock','Invalid motion interval.');
    ...
    bridge=@(stamp) receiverClockTime(clock,stamp)-startTime;
    ...
    metadata=struct('source',"/vehicle/wheel_speed_report: four-wheel fusion with steering/IMU", ...
        'clockModelId',string(clock.modelId),'wheelFile',path,'wheelPackets',numel(wt),'configuration',cfg, ...
