...
def main():
    ...
    # Subtract whole weeks separately to avoid losing sub-microsecond precision.
    receiver=(ins.gps_week.to_numpy()-ins.gps_week.iloc[0])*604800+ins.gps_seconds.to_numpy()-ins.gps_seconds.iloc[0]
    start=np.interp(frames.stamp_sec.iloc[0]-origin,ins.stamp_sec.to_numpy()-origin,receiver)
    time=(best.gps_week.to_numpy()-ins.gps_week.iloc[0])*604800+best.gps_seconds.to_numpy()-ins.gps_seconds.iloc[0]-start
    ...
    out=ROOT/'output/mncav_synchronous_bestpos_20260917';out.mkdir(parents=True,exist_ok=True)
    ...
    metadata=dict(source='/novatel/oem7/bestpos',samples=len(time),medianRateHz=float(1/np.median(np.diff(time))),
        ...
        clock='BESTPOS GPS week/seconds directly; INSPVA timestamps only locate the first LiDAR epoch',
